...
get_itemized_contributions <- function(
  ...
  if (!is.null(data)){
    ...
    if (is.null(committee_id)){
      ...
      if ('committee_id' %in% names(data)){
        ...
        committee_id <- data[['committee_id']] %>% unique()
  ...
  names(query_parameters) <- names(query_parameters) %>% str_remove_all('[0123456789]')

  query_parameters <- as.list(query_parameters)

  responses[[1]] <- get_openfec(path = paste0("/schedules/schedule_a/"), query = query_parameters)
  ...
  #
  #     responses[[c_id]][[i]] <- get_openfec(path = paste0("/schedules/schedule_a/"), query = query_parameters)
  #     print(i)
  #
  #   }
  #
