...
search_candidates <- function(data_structure = 'both', candidate_id = NULL,
                              ...
                              federal_funds_flag = NULL, has_raised_funds = NULL,
                              sort_hide_null = NULL, office = NULL, election_year = NULL,
                              sort_null_only = NULL, incumbent_challenge = NULL, cycle = NULL,
  ...
  tidy_candidates <- purrr::map(responses, function(x) x$results) %>%
    unlist(recursive = F) %>%
    tibble(
      load_date = map_chr(. , "load_date"),
      last_file_date = map_chr(. , "last_file_date"),
      candidate_id = map_chr(. , "candidate_id"),
      election_years = map(. , "election_years"),
      party = map_chr(. , "party"),
      candidate_status = map_chr(. , "candidate_status"),
      office_full = map_chr(. , "office_full"),
      state = map_chr(. , "state"),
      federal_funds_flag = map_lgl(. , "federal_funds_flag"),
      has_raised_funds = map_lgl(. , "has_raised_funds"),
      office = map_chr(. , "office"),
      election_districts = map(. , "election_districts"),
      name = map_chr(., "name"),
      principal_committees = map(. , "principal_committees"),
      district_number = map_int(. , "district_number"),
      cycles = map(. , "cycles"),
      incumbent_challenge = map_chr(. , "incumbent_challenge"),
      party_full = map_chr(. , "party_full"),
      first_file_date = map_chr(. , "first_file_date"),
      active_through = map_int(. , "active_through"),
      incumbent_challenge_full = map_chr(. , "incumbent_challenge_full"),
      last_f2_date = map_chr(. , "last_f2_date"),
      district = map_chr(. , "district")
    ) %>%
